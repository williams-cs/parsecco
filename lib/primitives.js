...
(function (Primitives) {
    ...
    function item() {
        return (istream) => {
            if (istream.isEmpty()) {
                let error_mes = "No more character";
    ...
    function sat(char_class) {
        let f = (x) => {
            return (char_class.indexOf(x.toString()) > -1)
                ? result(x)
                : (istream) => new Failure(istream, istream.startpos - 1);
        };
        return bind(item())(f);
    }
    Primitives.sat = sat;
    ...
    function eof() {
        return (istream) => {
            return istream.isEOF() ? new Success(istream, Primitives.EOF) : new Failure(istream, istream.startpos);
        };
    }
    Primitives.eof = eof;
    /**
