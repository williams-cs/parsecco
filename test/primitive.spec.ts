...
describe('Recparser', () => {
  it('should expand just like an ordinary parser, even for recursive definitions.', function* () {
    ...
    let pImpl = res[1];
    pImpl.contents = P.pipe2<CU.CharStream, CU.CharStream, CU.CharStream>(P.item)(p)(
      (a, b) => new CU.CharStream(a.toString() + b.toString())
    );
...
describe('Seq parser', () => {
  it('should successfully apply two parsers in a row', function* () {
    const output = yield* P.seq<CU.CharStream, CU.CharStream>(P.item)(P.item)(inputstream);
    const expected = ['h', 'e'];
    switch (output.tag) {
      case 'success':
...
describe('Prefix parser', () => {
  it('should allow parsing alternatives that share a prefix (case 1)', function* () {
    const outcome = yield* P.prefix(P.str('hello'))(P.str('bacon'))((p, s) => assert.fail())(inputstream);
    switch (outcome.tag) {
      case 'success':
  ...
  it('should allow parsing alternatives that share a prefix (case 2)', function* () {
    ...
    const outcome = yield* P.prefix<CU.CharStream, CU.CharStream>(P.str('hello'))(P.str('bacon'))(
      (p, s) => new CU.CharStream(p.toString() + s.toString())
    )(input);
    switch (outcome.tag) {
      case 'success':
  ...
  it('should only ever run the first parser once (case 1)', function* () {
    ...
    const outcome = yield* P.prefix(first)(P.str('bacon'))((p, s) => assert.fail())(inputstream);
    switch (outcome.tag) {
      case 'success':
        expect(outcome.result.toString()).to.equal('hello');
  ...
  it('should only ever run the first parser once (case 2)', function* () {
    ...
    const input = new CU.CharStream('hellobacon');
    const outcome = yield* P.prefix<CU.CharStream, CU.CharStream>(first)(P.str('bacon'))(
      (p, s) => new CU.CharStream(p.toString() + s.toString())
    )(input);
    switch (outcome.tag) {
      case 'success':
...
describe('pipe2 parser', () => {
  it('should not eagerly compose two parsers', () => {
    let p2: P.IParser<CU.CharStream> = i =>
      P.pipe2<CU.CharStream, CU.CharStream, CU.CharStream>(P.char('.'))(p2)(x => x)(i);
    assert(true);
  });

  it('should apply a function to the result of two successful parses in sequence', function* () {
    const i = new CU.CharStream('ok');
    const f = (a: CU.CharStream, b: CU.CharStream) => a.toString() + b.toString();
    const output = yield* P.pipe2<CU.CharStream, CU.CharStream, string>(P.item)(P.item)(f)(i);
    switch (output.tag) {
      case 'success':
...
describe('pipe3 parser', () => {
  it('should apply a function to the result of three successful parses in sequence', function* () {
    ...
    const f = (a: CU.CharStream, b: CU.CharStream, c: CU.CharStream) => a.toString() + b.toString() + c.toString();
    const output = yield* P.pipe3<CU.CharStream, CU.CharStream, CU.CharStream, string>(P.item)(P.item)(P.item)(f)(i);
    switch (output.tag) {
      case 'success':
...
describe('EOF parser', () => {
  it('should succeed at the end of the input', function* () {
    const p = P.pipe2<CU.CharStream, P.EOFMark, CU.CharStream>(P.str('helloworld'))(P.eof)((a, b) => a);
    const output = yield* p(inputstream);
    switch (output.tag) {
      case 'success':
...
describe('matchWhileCharCode', () => {
  it('should match until a predicate returns false', function* () {
    const input = new CU.CharStream('helloFunction3(');
    const output = yield* P.matchWhileCharCode(n => (n > 48 && n < 58) || (n > 65 && n < 91) || (n > 97 && n < 123))(
      input
    );
