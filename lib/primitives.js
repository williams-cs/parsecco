...
const StringError_1 = require("./StringError");
const BetweenLeftError_1 = require("./BetweenLeftError");
const BetweenRightError_1 = require("./BetweenRightError");
var Primitives;
(function (Primitives) {
    class EOFMark {
        constructor() { }
    ...
    function item() {
        return (istream) => {
            if (istream.isEmpty()) {
                return new Failure(istream, istream.startpos, new CharError_1.CharError(""), true);
            }
            else {
    ...
    function sat(char_class) {
        let f = (x) => {
            return (char_class.indexOf(x.toString()) > -1)
                ? result(x)
                : (istream) => new Failure(istream, istream.startpos - 1, new CharError_1.CharError(x.toString()));
        };
        return bind(item())(f);
    }
    Primitives.sat = sat;
    ...
    function between(popen) {
        return (pclose) => {
            return (p) => {
                let l = expect(left(p)(pclose))(new BetweenRightError_1.BetweenRightError("("));
                let r = expect(right(popen)(l))(new BetweenLeftError_1.BetweenLeftError(")"));
                return r;
            };
        };
    }
